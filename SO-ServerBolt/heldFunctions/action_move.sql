...
BEGIN
    ...
    -- If movement distance exceeds unit movement range, raise an exception with detailed message
        RAISE EXCEPTION 'Unit cannot move to position (%, %) from (%, %): movement distance exceeds range (% > %)',
            p_new_x, p_new_y, p_old_x, p_old_y, v_movement_distance, v_unit_attrs.movement_range;
    END IF;
