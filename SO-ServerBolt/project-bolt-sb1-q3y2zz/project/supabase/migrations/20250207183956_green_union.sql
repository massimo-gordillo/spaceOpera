-- Create function to handle health updates according to the game logic
...
BEGIN
    ...
    v_health_threshold := (p_max_health * 0.05)::INTEGER;
    ...
    v_new_health := v_new_health - (v_new_health % (p_max_health / 100));
    
    RETURN v_new_health;
END;
$$;

-- Update action_combat to use the new health calculation
...
BEGIN
    ...
    -- Apply damage to defender using new health calculation
    ...
    IF v_can_counterattack THEN
        ...
        -- Apply counterattack damage using new health calculation
        v_new_health := combat_update_health(
            v_attacker.id,
            v_damage::INTEGER,
