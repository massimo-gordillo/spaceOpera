/*
  # Update match pieces system
  
  1. Changes
    - Convert match_piece_list from partitioned to regular table
    - Update create_match function to populate match pieces from map
    
  2. Details
    - Drop existing partitioned table
    - Create new regular table with same structure
    - Add indexes and RLS policies
    - Update create_match to copy pieces from map_init_pieces
*/
...
BEGIN
  ...
  -- Copy pieces from map_init_pieces to match_piece_list
  ...
  SELECT
    ...
    type_num,
    CASE 
      -- Remap player controllers based on random player assignment
      WHEN player_controller = 0 THEN 0  -- Neutral pieces stay 0
      WHEN player_controller = 1 THEN    -- Player 1 pieces
        CASE WHEN player1 = player_a_id THEN 1 ELSE 2 END
      WHEN player_controller = 2 THEN    -- Player 2 pieces
        CASE WHEN player1 = player_a_id THEN 2 ELSE 1 END
