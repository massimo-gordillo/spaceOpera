-- Update submit_game_actions to calculate and store game state hash
...
BEGIN
    ...
    BEGIN
        ...
        -- Calculate new game state hash after all actions are processed
        v_new_hash := match_calculate_game_state_hash(p_match_id);

        -- End turn, switch current player, and update hash
        UPDATE matches
