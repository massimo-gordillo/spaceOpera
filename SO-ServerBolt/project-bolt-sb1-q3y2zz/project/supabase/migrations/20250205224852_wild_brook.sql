...
BEGIN
    ...
    IF v_attacker_piece.unit_id != p_unit_id_num THEN
        RAISE EXCEPTION 'Unit type mismatch: Expected %, found %', p_unit_id_num, v_attacker_piece.unit_id;
    END IF;
    ...
    WHERE gamePieceId = p_unit_id_num;
    ...
    FROM unit_health_types
    WHERE unit_id = v_defender_piece.unit_id;

    IF v_health_type IS NULL THEN
        RAISE EXCEPTION 'No health type found for defender unit type %', v_defender_piece.unit_id;
    END IF;
    ...
    IF v_multiplier IS NULL THEN
        RAISE EXCEPTION 'No combat multiplier found for damage type % and health type %', 
            v_damage_type_id, v_health_type;
    END IF;
